...
Px2rem.prototype.generateRem = function (cssText) {
  ...
  function processRules(rules, noDealPx) { // FIXME: keyframes do not support `force px` comment
    ...
    for (var i = 0; i < rules.length; i++) {
      ...
      for (var j = 0; j < declarations.length; j++) {
        var declaration = declarations[j];
        var filterProperty = self._filterProperty(declaration.property);
        // need transform: declaration && has 'px'
        if (declaration.type === 'declaration' && pxRegExp.test(declaration.value)) {
          var nextDeclaration = rule.declarations[j + 1];
...
Px2rem.prototype._getCalcValue = function (type, value, dpr) {
  ...
  return value.replace(pxGlobalRegExp, function ($0, $1) {
    var value = "";
      if(config.minPixelValue && config.minPixelValue < $1 || !config.minPixelValue)
        value = type === 'px' ? getValue($1 * dpr / config.baseDpr) : getValue($1 / config.remUnit);
      else 
        value = $0;
...
Px2rem.prototype._filterProperty = function(property){
  var config = this.config;
  var isFilter = false;
  if(config.filterProperties.length)
  for (var i = 0; i < config.filterProperties.length; i++) {
    if(config.filterProperties[i] === property){
      isFilter = true;
    }
  }
  return isFilter;
